...
async function review(e){
  ...
  try{
    const {data:existing,error:lookupError}=await sb.from('product_reviews')
      .select('id')
      .eq('product_id',id)
    ...
    const query=existing?.id
      ? sb.from('product_reviews').update({rating,body,updated_at:new Date().toISOString()}).eq('id',existing.id)
      : sb.from('product_reviews').insert({product_id:id,author_id:currentUser.id,rating,body});
    const {error}=await query;
    if(error)throw error;

    form.reset();
    toast(existing?.id?'Review updated.':'Review posted.','success');
    await loadProduct();
  }catch(error){
    console.error('Review submission failed:',error);
    toast(error?.message||'Unable to post your review.');
  }finally{
